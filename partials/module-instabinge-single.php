<div id="modal-instabinge" class="modal modal-transparent modal-instabinge">
  <button class="modal-action modal-close"><i class="icon-close"></i></button>
  <div class="modal-wrap">
    <div class="modal-wrap-row">
      <div class="modal-view frame">
        <div id="modal-instabinge-frame" class="modal-instabinge-frame"></div>
      </div>
      <div id="modal-instabinge-controls" class="sly-controls">
        <button class="sly-prev" type="text"><i class="icon-arrow-left"></i></button>
        <button class="sly-next" type="text"><i class="icon-arrow-right"></i></button>
      </div>
    </div>
  </div>
  <img class="modal-loader-icon" src="<?php bloginfo('template_directory'); ?>/static/images/loader.GIF " />
</div>